...
const Home = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    ...
    for (let index = 0; index < numberOfBlastHits; index++) {
      const currentBlastResist = Math.floor(
        targetMonsterInitialBlastResistanceValue *
          Math.pow(1.3, currentBlastHits)
      );
      console.log(`이전의 폭파 판정까지 폭파는 ${currentBlastHits}번 터짐`);
      ...
      if (diffBtwCurrentBlastResistanceAndCurrentCumulativeBlast <= 0) {
        ...
        console.log(`이제 폭파 ${currentBlastHits}번 터짐`);
      } else {
    ...
    console.log(`${currentBlastHits}번 폭파 가능`);
    setBlastCount(currentBlastHits);
    const totalDamage =
      Math.floor(
